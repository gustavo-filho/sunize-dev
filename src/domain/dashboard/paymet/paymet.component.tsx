...
export function Payment() {
    ...
    const { data, error } = useFetch(
        `products/${productId}?checkout=${Cookies.get(`${productId}@isClicked`) !== 'true' ? 'true' : 'false'
        }`,
    )
    ...
    useEffect(() => {
        ...
        if (error) history.replace('/dashboard/mercado')
        // eslint-disable-next-line react-hooks/exhaustive-deps
        ...
        const timer = setInterval(async () => {
            if (isToastAllowed) {
                await new Audio('/notification.mp3')
                    .play()
                    .then(() => {
                        toast.warn(`${customCheckout?.notifications.people_buy_product_today.qtd_max} pessoas adquiriram este produto nesta semana.`, {
                            toastId: idToast
                        })
                        clearInterval(timer)
                    })
    ...
    useEffect(() => {
        ...
        if (voucher && product.price) {
            applyVoucher({ voucher: voucher, productId })
        }
