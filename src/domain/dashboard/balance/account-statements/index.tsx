...
export function AccountStatements(): JSX.Element {
  ...
  const [dataResponseInit, setDataResponseInit] = useState<
    dataResponseInitType[]
  >([]);
  ...
  async function functionListDeposits(): Promise<void> {
    ...
    const response7 = await api.get(
      `/users/${
        user!.id
      }/listDeposits?month=${currentMonth}&year=${currentYear}`,
    );
    const response6 = await api.get(
      `/users/${user!.id}/listDeposits?month=${month6}&year=${currentYear}`,
    );
    const response5 = await api.get(
      `/users/${user!.id}/listDeposits?month=${month5}&year=${currentYear}`,
    );
    const response4 = await api.get(
      `/users/${user!.id}/listDeposits?month=${month4}&year=${currentYear}`,
    );
    const response3 = await api.get(
      `/users/${user!.id}/listDeposits?month=${month3}&year=${currentYear}`,
    );
    const response2 = await api.get(
      `/users/${user!.id}/listDeposits?month=${month2}&year=${currentYear}`,
    );
    const response1 = await api.get(
      `/users/${user!.id}/listDeposits?month=${month1}&year=${currentYear}`,
    );

    const listFilter7 = response7.data.body.Deposits.filter(
      (itemFilter: dataResponseInitType) => {
    ...
    const listFilter6 = response6.data.body.Deposits.filter(
      (itemFilter: dataResponseInitType) => {
        if (itemFilter.Message === 'Pagamento Efetivado') {
          return itemFilter;
        }
    ...
    const listFilter5 = response5.data.body.Deposits.filter(
      (itemFilter: dataResponseInitType) => {
        if (itemFilter.Message === 'Pagamento Efetivado') {
          return itemFilter;
        }
    ...
    const listFilter4 = response4.data.body.Deposits.filter(
      (itemFilter: dataResponseInitType) => {
        if (itemFilter.Message === 'Pagamento Efetivado') {
          return itemFilter;
        }
    ...
    const listFilter3 = response3.data.body.Deposits.filter(
      (itemFilter: dataResponseInitType) => {
        if (itemFilter.Message === 'Pagamento Efetivado') {
          return itemFilter;
        }
    ...
    const listFilter2 = response2.data.body.Deposits.filter(
      (itemFilter: dataResponseInitType) => {
        if (itemFilter.Message === 'Pagamento Efetivado') {
          return itemFilter;
        }
    ...
    const listFilter1 = response1.data.body.Deposits.filter(
      (itemFilter: dataResponseInitType) => {
        if (itemFilter.Message === 'Pagamento Efetivado') {
          return itemFilter;
        }
  ...
  async function filterModal(): Promise<void> {
    try {
      const response = await api.get(
        `/users/${
          user!.id
        }/listDeposits?month=${monthFilter}&year=${yearFilter}`,
      );
      setDataResponseInit([]);
      if (response) {
        setDataResponseInit(
          response.data.body.Deposits.filter(
            (itemFilter: dataResponseInitType) => {
              if (itemFilter.Message !== 'Pagamento Efetivado') {
                return itemFilter;
              }
  ...
  return (
    <Container>
      ...
      <div className="wrapperNavigation">{wrapperNavigation()}</div>

      <div
        style={{ backgroundColor: '#27293d', borderRadius: '0 6px 6px 6px' }}
      >
        ...
        {init && (
          ...
          >
            {' '}
            Carregando os dados, favor aguarde...
          </Typography>
        )}
      ...
      <Dialog open={modalFilter} onClose={() => setModalFilter(false)}>
